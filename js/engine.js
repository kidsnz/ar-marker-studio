...
(function (root) {
  ...
  /**
   * 【撤回した基準】かつてここに「散らばり17%以上で安定」という線を置いていたが、
   * **根拠が無かったので撤回した**（2026-08-15）。実機で答えが分かっている7例:
   *
   *   emoticons/01(旧)  2点 /  0.0%  … 揺れる     ← 唯一3点を割っている
   ...
   *
   * **不安定なのは1本だけで、それは唯一 3点を割っていたもの。**
   * 安定なものの散らばりは 1.8%〜22.4% とばらばらで、何も予測していない。
   * ガイドの「1%/17%/33%」は satoshi の3枚で点数と散らばりが一緒に動いていたため、
   * 相関を因果と取り違えていた。**判定はソース由来の3点の下限だけで行う。**
   * 散らばりは参考値として出す（同じ点数でも固まっていれば姿勢は決まりにくい、
   * という ar2SelectTemplate の理屈自体は正しい）。
   */
  var STABLE_SPREAD = 0.17;   // 参考表示にのみ使う。合否には使わない
