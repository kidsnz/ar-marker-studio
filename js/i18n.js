...
(function (root) {
  ...
  var JA = {
    ...
    'pick.profileNote': 'マーカーが画面幅の何割を占めるかごとの追従点:',
    ...
    'res.provisional': '<strong>「安定」「不足ぎみ」は暫定。</strong>'
      + '{good}点・{poor}点というしきい値は、このツールがいま使っているのと'
      + '同じ約181pxの幅に対して較正されているので引き継げる。ただし根拠にした'
      + '実機メモに未解決の食い違いがある（安定と記録されたマーカーが、'
      + '素のARnftの黒帯前提では2点、つまり追従が止まる{floor}点を割ってしまう）。'
      + 'ソースで裏が取れているのはその{floor}点の下限（tracking.c）だけ。',
