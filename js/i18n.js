...
(function (root) {
  ...
  var JA = {
    ...
    'cfg.dist': '見る距離',
    'cfg.fillIs': '→ 画面の {near}〜{far}% を占める（画面上 {npx}〜{fpx}px）',
    'cfg.dist.sub': 'トラッカーが実際に見ているのは「画面の何割を占めるか」で、'
      + '距離の横に出している。実測したところ、3つの使い方は'
      + '<strong>同じ場所に来る</strong>。どれも画面幅の100〜120%、'
      + '画面上でおよそ185〜225px（3つとも実機で距離を実測して確認）。'
      + '5.5cmのステッカーを8cmから見るのと、'
      + '72インチの絵を2.9mから見るのは、トラッカーには同じ大きさ。'
      + '一定なのは構え方の方で、実寸は「どこに立つか」を決めているだけ。'
      + 'だから狙う的は3つではなく1つ。',
    'cfg.presets': 'よくある実寸:',
