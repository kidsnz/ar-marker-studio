...
(function (root) {
  ...
  var JA = {
    ...
    'meta.scale': '拡大',
    'meta.scaleV': '{k}倍・ニアレストネイバー（元は {w}×{h}）',
    ...
    'cfg.leveli1': '1（既定）',
    'cfg.uses': 'どこで使う？:',
    'cfg.uPaint': '絵 72インチ・2.7〜3.1m',
    'cfg.uScreen': 'ラップトップ画面 12cm・18〜22cm',
    'cfg.uSticker': 'ステッカー 5.5cm・7.5〜9cm',
    'cfg.standAt': '→ この実寸なら {near}〜{far} の距離で使う（そこで {lo}〜{hi}px になる＝狙う的）',
    'cfg.dist.sub': '距離を入れる必要はない。マーカーは毎回'
      + '「画面がだいたい埋まる」ところまで寄せて使っていて、'
      + 'トラッカーが見ているのはそれだけ。実測すると、'
      + '5.5cmのステッカーを8cmから、ラップトップ画面の12cmを20cmから、'
      + '72インチの絵を2.9mから見るのは<strong>全部同じ大きさ</strong>で、'
      + '画布上185〜225pxになる。だから以下はその1つの的で採点する'
      + '（余裕を見て<strong>150〜250px</strong>）。'
      + '実寸が決めるのは「どこに立つか」だけで、それが上の行。',
    'cfg.presets': 'よくある実寸:',
    ...
    'warn.tooBig': 'この画像は {w}×{h}（{mp}メガ画素）で、マーカーには大きすぎる。'
      + 'トラッカーが見るのはどんなに大きくても画面上250px程度なので、'
      + 'それ以上の画素は「一度も使われない距離帯」を増やすだけ。'
      + 'それでも .iset と .fset3 には入るのでファイルは大きく膨らみ、生成にも数分かかる。'
      + '<strong>先に幅 {rec}px 程度まで縮めることをすすめる。</strong>',
    ...
    'search.skipped': '{list} は試していない。拡大すると {man}万画素を超えて'
      + 'ブラウザのメモリが足りなくなるため。',
    'search.region': '上で設定した距離で、しかも画面に写っているぶんだけを数えて採点している。',
    'pick.best': '性能重視',
    'pick.balanced': 'バランス',
    'pick.small': 'サイズ重視',
    'pick.set': '{k}倍 / -level={lv} / -dpi={dpi}　（{w}×{h}）',
    'pick.size': '合計 {kb} KB　（.fset {f} ／ .fset3 {f3} ／ .iset {i} KB）',
    'pick.profileNote': '距離ごとの追従点:',
    'why.best': '使える距離 {c}/{n}・散らばり {s}%。試した {tried} 通りで最良',
    'why.balanced': '使える距離は性能重視と同じ {c}/{n}、散らばりは {s}%（性能重視は {s2}%）で {d}KB 小さい',
    'why.balanced.eq': '使える距離 {c}/{n}・散らばり {s}% は性能重視と同じまま、{d}KB 小さい',
    'why.balanced.same': 'これ以上小さくして品質を保てるものが無いので、性能重視がそのままバランス案',
    'why.small': '使える距離を1つ落として {c}/{n}。バランス案より {d}KB 小さい',
    'why.small.eq': '使える距離は {c}/{n} のまま、散らばりが {s}%（バランス案は {s2}%）で {d}KB 小さい',
    'why.small.same': 'これより小さくする価値のあるものが無いので、バランス案がいちばん小さい',
    'btn.prose': '説明を表示',
    'btn.proseHide': '説明を隠す',
    'tbl.fold': '距離帯ごとの内訳を見る',
    'dist.fold': '距離ごとの内訳を見る',
    'btn.use': 'この設定にする',
    'btn.dlAll': '▼ 3つまとめて',
    'pick.applied': '反映した。作業中の画像を {k}倍にし、-dpi と -level を入れた。'
      + '3つのファイルは生成済みなので、上のボタンでそのまま落とせる。',
    'sr.scale': '拡大',
    'sr.px': '画素数',
    'sr.cov': '使える距離',
    'sr.pts': '距離ごとの点数',
    ...
    // --- 4. 判定 ---
    'step3.title': '判定',
    'tbl.caption': '距離帯ごとの追従点。帯の画像の幅(px)は、'
      + 'その帯を使うときにトラッカーが見ているマーカーの幅と同じ数なので、'
      + 'そのまま「マーカーがどれだけ大きく写るか」を表す。'
      + '<strong>色の付いた行が実際に使える窓</strong>'
      + '（マーカーが画面に収まっていて、かつ3点以上ある帯）。',
    'tbl.dist': '距離',
    'tbl.tooClose': '近すぎて画面からはみ出す',
    'res.tooSmall': 'さらに {n} 段は表から省いた。画像が {ts}px 以下でテンプレートが収まらず、'
      + '1点も置けない帯だから。本物の生成器と照合しても、この帯は全部0点だった。',
    'tbl.px': '画素数',
    'tbl.pts': '点',
    ...
    'res.points': '点',
    'res.onScreen': '画面に {px}px で写る →',
    'res.coverYes': '<strong>3パターン全部いける。</strong>'
      + '狙う的の全域で3点以上あるので、同じファイルが絵でもステッカーでもサイトでも使える。',
    'res.coverNo': '<strong>的の全域はカバーできていない。</strong>'
      + '{total}箇所のうち{n}箇所で追従が止まる（この実寸だと {from}〜{to} の距離）。'
      + '生成前に絵を拡大するか、マーカーを大きく刷る。',
    'res.coverWeak': '<strong>追従はするが、安定はしない。</strong>'
      + '{total}箇所のうち{n}箇所で散らばりが{ss}%に届いていない'
      + '（最悪は {at} で {p}点・{s}%）。実機の実測ではここは姿勢が揺れる領域。'
      + '生成前に絵を拡大すると効く。',
    'res.thinNote': 'いちばん細いのは {at} で {p}点・{s}%。'
      + 'これは参考程度。実機と突き合わせた限り、散らばり単独では何も予測できていない'
      + '（効いていたのは3点の下限の方）。',
    'pick.coverYes': '3パターン全部いける',
    'pick.coverNo': '的の {total} 箇所中 {n} 箇所',
    ...
    'verdict.limit': 'ギリギリ',
    'verdict.cannot': '追従不可',
    'res.detail': '散らばり {spread}% ／ フォールバック込み {fb} 点',
    'res.detailCrop': '散らばり {spread}% ／ '
      + 'マーカーが画面より大きいので中央 {vis}% しか数えていない（全体では {all} 点）'
      + ' ／ フォールバック込み {fb} 点',
    ...
    'res.spreadNote': '散らばりは、点が囲む面積がマーカー全体の何割かを表す。'
      ...
      + 'マーカーが画面より大きいときは、見えている中央だけで測る'
      + '（実機と一致したのはこの測り方だった）。',
    'res.basis': 'ARnft が画像処理する画布は 320×240 固定（ARnft.js の prepareImage）。'
      + '縦持ちでは ARnft-rot が映像を90度回すので、素の ARnft の 180×240 ではなく '
      + '240×320 が使える。さらに映像は object-fit: cover で表示されるため約1/4が'
      + '画面の外にあり、実際に判定に使えるのは 181×320。'
    ...
    'gen.running': '生成中… {t}',
    'gen.done': '生成した（{s} 秒）。3つとも落としている。同じフォルダに置いて使う。'
      + '初回だけブラウザが「複数ファイルのダウンロードを許可しますか」と聞くので、'
      + '許可すれば以後は聞かれない。届かなかったぶんは下のリンクから取れる。',
